...
├── docs/lab-guide.md        # the full lab guide — 22 labs, 4 demos, final project
...
There are **no branches to juggle** — `solutions/` is a normal folder:
- **Day 1** (Labs 1–10) is CLI-only → `solutions/day-1/` is an answer key (expected
  output + resolved Stop & think).
- **Days 2–3** → one folder per lab with the finished artifact(s).

A stuck student compares rather than copies, e.g.:
```bash
diff app/Dockerfile solutions/lab-13-multistage/Dockerfile
```

**Gated release (optional):** the Stop & think answers are pedagogically load-bearing.
If you want to withhold solutions until after each day, keep `solutions/` in a separate
access-controlled repo (or a private branch) and open access on your schedule. The rest
of the repo can stay open to students from day one.
