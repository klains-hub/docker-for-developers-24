...
- Solutions live in the `solutions/` folder of the course repo. Day 1 is a CLI answer key (`solutions/day-1/`); later labs have a folder each. Use them only after you have tried.
...
> **Instructor note — two images to publish once:** Labs 5 and 6 run pre-built images. Build and push them before the course from the `lab-images/` bundle: `levep79/broken-demo` (Lab 5) and `levep79/course-app` (Lab 6, and the same source students build in Labs 11–14). See `lab-images/README.md`. Everything else is an official image or built by students.
...
> **Prerequisite (instructor):** publish `levep79/broken-demo` before class — see `lab-images/README.md`.
...
**Cleanup:** `docker rm -f web`  (the next lab reuses the name `web` and port 8080)
...
## Lab 14 — Containerize the Course App
...
## Lab 20 — Scan & Harden
**~45 min · Goal:** Find real vulnerabilities in your image and reduce them — using **Trivy** (you don't have Docker Scout).

1. Scan the image (Trivy runs as a container — nothing to install). First run downloads the vuln DB; the cache mount makes later runs fast:
   ```bash
   $ mkdir -p ~/trivy-cache
   ...
   ```
2. Focus on what's actionable — fixable HIGH/CRITICAL, and note the count:
   ```bash
   $ docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
       -v ~/trivy-cache:/root/.cache/trivy \
       aquasec/trivy image --severity HIGH,CRITICAL --ignore-unfixed myapp:1.0
   ```
3. Harden the Dockerfile — drop root and add only what's needed:
   ```dockerfile
   COPY --chown=node:node . .
   USER node
   ```
4. Switch to a slimmer / newer base image (e.g. `node:20-alpine` instead of `node:20`) and rebuild.
5. Re-scan and compare the numbers before vs after.

**Expected result:** A measurably lower HIGH/CRITICAL count and a non-root runtime.
...
**Pitfall:** `docker scan` / the old Snyk integration is retired, and `docker scout` ships only with Docker Desktop. **Trivy** needs nothing installed — run it as a container as above. (If you later get Scout, `docker scout quickview` / `cves` / `recommendations` are the equivalents.)
...
> **Prerequisite:** start from your Lab 14 / Lab 20 production image.
...
4. A **Trivy** scan with the **worst findings addressed**.
...
- **Supply-chain security:** SBOMs, signing, and image scanning (Trivy) as a CI gate.
- **Compose depth:** profiles, `include`, and `docker compose watch` for fast inner-loop dev.

*Solutions live in `solutions/` — a CLI answer key in `solutions/day-1/`, and a folder per lab for Days 2–3.*